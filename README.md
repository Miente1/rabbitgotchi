...
- Needs: Hunger, Happiness, Energy, Health. Neglect them and it gets sick or dies. **While asleep, hunger/poop/health run at 1/3 the awake pace**, energy recharges (instead of draining), and happiness holds steady. **Sickness is deferred** — the roll still happens overnight, but the illness is only held pending and *takes hold when the pet wakes up* ("Woke up feeling sick..."), so it can never silently drain health overnight. **Poop deals no health damage while asleep** either (it still piles up and still needs cleaning on waking). That leaves hunger as the *only* possible sleep-death path — and at 1/3 pace, full hunger takes ~18h to empty, so **a pet put to bed with full bars is guaranteed to survive 16h+ asleep** regardless of luck. (Putting it to bed already hungry can still cost it, just slowly.)
...
- **Sickness**: an untreated illness drains health to 0 in **~3 hours** (a fair window to notice and give 💊 Medicine — not the old brutal 25 minutes). Sickness damage is paused entirely while asleep, and a sickness that rolls overnight is deferred until the pet wakes ("Woke up feeling sick..."), so it can never kill during sleep.
- **Feed**: opens a picker with 🍔 Meal, 🍪 Snack, and ↩ Back; scroll to move the highlight, press to confirm. Meal restores more hunger and adds more weight; Snack restores less hunger but gives a small happiness bump. Back closes the picker without feeding (so opening Feed by accident costs nothing).
- **Play minigame**: best of 5 rounds. Each round shows a number 1–9; scroll to pick Higher or Lower than the next number, press to lock in the guess. The first round also offers ↩ Back to bail out with no cost; the energy/weight cost is only charged once you make your first actual guess. Each round pops a "🎉 Correct!" celebration or "💢 Wrong!" irritation animation in the modal. After all 5 rounds, the overall record (not each round) drives the happiness change: win the match (3+/5) → +14 to +22 happiness depending on how decisively; lose the match → −14 to −22. Energy/weight cost is paid once per match, not per round. Winning the overall match also gets the pet itself to do a visible victory-dance animation on screen once the modal closes (a bigger repeating bounce, not just modal text); losing gets a slumped "sad" animation (dip, hold with a little sobbing tremor, recover) plus its own toast and sound, mirroring the win side.
- Tapping/petting the pet directly gives it a bigger happy bounce (same size as the mood-reaction bounce, not the small idle hop) plus a random happy speech-bubble line, not just a silent happiness nudge.
- **Calls & Naughty (false calls)**: the pet calls out what it needs (hunger/energy/happiness) using the same speech-bubble lines whether the call is genuine or not — hunger and happiness already had these, energy now has its own "I'm so sleepy..." pool too. Scheduled reliably about **once a day** (not a random low-frequency roll), the pet sometimes falsely cries about whichever need is actually the *fullest* right now, and also refuses to eat until disciplined. The specific false claim (not just a generic "hmph") keeps re-appearing every ~25–40s via the normal ambient chatter for as long as it's unresolved, so a brief check-in during the 3-hour window is likely to catch the exact claim, not just a vague "it's cranky" cue. It's also extra grumpy the whole time: happiness drains at **2x** the normal rate while naughty. You have to check ✋ Discipline to see which stat it's crying about and that stat's actual bar value to catch the lie. Catching it in time (within the 3-hour window) costs a little happiness (−3) and fills a **discipline bar** (capped at 5, shown until adulthood). **Missing the window entirely counts as a real mistake** — added straight to the same mistake counter neglect uses, no discipline gained. At the day-5 checkpoint, `mistakes - discipline` decides the evolution/lifespan tier (see above), so consistently catching false calls offsets earlier neglect, while missing them (or not disciplining enough) pushes toward a lesser Tamagotchi both ways. Disciplining when it's *not* naughty is also a real mistake: **−15 happiness**, no discipline-bar gain — the confirm screen shows the claim and the real bar value first specifically so this is an informed choice, not a misclick.
