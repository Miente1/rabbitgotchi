...
- **Play minigame**: best of 5 rounds. Each round shows a number 1–9; scroll to pick Higher or Lower than the next number, press to lock in the guess. Each round pops a "🎉 Correct!" celebration or "💢 Wrong!" irritation animation in the modal. After all 5 rounds, the overall record (not each round) drives the happiness change: win the match (3+/5) → +14 to +22 happiness depending on how decisively; lose the match → −14 to −22. Energy/weight cost is paid once per match, not per round. Winning the overall match also gets the pet itself to do a visible victory-dance animation on screen once the modal closes (a bigger repeating bounce, not just modal text); losing gets a slumped "sad" animation (dip, hold with a little sobbing tremor, recover) plus its own toast and sound, mirroring the win side.
