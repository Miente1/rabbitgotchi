...
- **Calls & Naughty (false calls)**: the pet calls out what it needs (hunger/energy/happiness) using the same speech-bubble lines whether the call is genuine or not — hunger and happiness already had these, energy now has its own "I'm so sleepy..." pool too. Scheduled reliably about **once a day** (not a random low-frequency roll), the pet sometimes falsely cries about whichever need is actually the *fullest* right now, and also refuses to eat until disciplined. You have to check ✋ Discipline to see which stat it's crying about and that stat's actual bar value to catch the lie. Catching it in time (within a 3-hour window) costs a little happiness (−3) and fills a **discipline bar** (capped at 5, shown until adulthood). **Missing the window entirely counts as a real mistake** — added straight to the same mistake counter neglect uses, no discipline gained. At the day-5 checkpoint, `mistakes - discipline` decides the evolution/lifespan tier (see above), so consistently catching false calls offsets earlier neglect, while missing them (or not disciplining enough) pushes toward a lesser Tamagotchi both ways. Disciplining when it's *not* naughty is also a real mistake: **−15 happiness**, no discipline-bar gain — the confirm screen shows the claim and the real bar value first specifically so this is an informed choice, not a misclick.
