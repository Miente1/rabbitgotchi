...
- **Naughty**: scheduled (not a random low-frequency roll) to happen reliably about **once a day** — the pet refuses to eat until disciplined; Feed just bounces off with "It refuses to eat!". ✋ Discipline resolves it: costs a little happiness (−3) and fills a visible **discipline stat** (shown in the HUD as `disc:X/5` up until adulthood, capped at 5 — roughly one fillable point per day of the 5-day growth window). At the day-5 checkpoint, `mistakes - discipline` decides the evolution/lifespan tier (see above), so consistently catching naughty episodes can meaningfully offset earlier neglect, and not disciplining enough leaves the net mistake count high → a lesser Tamagotchi. Disciplining when it's *not* naughty is a real mistake: **−15 happiness**, no discipline-stat gain — spamming it is actively costly, not a free shortcut.
